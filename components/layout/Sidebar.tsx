...
export default function Sidebar() {
  ...
  return (
    <aside className="flex flex-col w-60 min-h-screen bg-card border-r border-border py-6 px-4">
      {/* Brand */}
      <div className="flex items-center gap-3 px-2 mb-8">
        ...
        <span className="font-bold text-sm tracking-wide">TradingPlatform</span>
      </div>
      ...
      <nav className="flex-1 space-y-1">
        ...
        {isAdmin && (
          <>
            ...
            <Link
              href="/admin/proyecciones"
              className={cn(
                'flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors',
                pathname.startsWith('/admin/proyecciones')
                  ? 'bg-primary/15 text-primary'
                  : 'text-muted-foreground hover:bg-secondary hover:text-foreground'
              )}
            >
      ...
      <div className="border-t border-border pt-4 mt-4 space-y-3">
        <div className="px-3">
          ...
          {isAdmin && (
            <span className="inline-block mt-1 text-xs bg-primary/15 text-primary px-2 py-0.5 rounded-full">Admin</span>
          )}
        </div>
        <button

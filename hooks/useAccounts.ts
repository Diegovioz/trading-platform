...
interface RawTrade { account_id: string; pnl: number; trade_date: string; created_at: string; }

function enrichAccount(
  raw: Record<string, unknown>,
  chronoTrades: RawTrade[],   // sorted oldest→newest
  ddType: 'static' | 'trailing',
  ddPercent: number,
): Account {
  const initial = Number(raw.initial_capital) || 0;
  const factor  = ddPercent / 100;
  const totalPnl = chronoTrades.reduce((s, t) => s + (t.pnl ?? 0), 0);
  ...
  if (ddType === 'trailing') {
    // Walk trades in time order to find true equity peak
    let running = initial;
    highestEquity = initial;
    for (const t of chronoTrades) {
      running += t.pnl ?? 0;
      if (running > highestEquity) highestEquity = running;
    }

    maxDD   = highestEquity * factor;
    ddFloor = highestEquity - maxDD;
    const lossFromPeak = highestEquity - balance;
    drawdownUsedPct = maxDD > 0 ? Math.min(100, Math.max(0, (lossFromPeak / maxDD) * 100)) : 0;

    console.log(`[DD trailing] balance=${balance.toFixed(2)} highestEq=${highestEquity.toFixed(2)} floor=${ddFloor.toFixed(2)}`);
  } else {
    highestEquity   = initial;
    maxDD           = initial * factor;
    ddFloor         = initial - maxDD;
...
export function useAccounts() {
  ...
  const fetchAccounts = useCallback(async () => {
    ...
    const [{ data: accs }, { data: trades }, { data: profile }] = await Promise.all([
      ...
      supabase.from('journal_trades')
        .select('account_id, pnl, trade_date, created_at')
        .eq('user_id', user.id)
    ...
    const enriched: Account[] = (accs ?? []).map(a => {
      const acTrades = (trades ?? [] as RawTrade[]).filter(t => t.account_id === a.id) as RawTrade[];
      return enrichAccount(a as Record<string, unknown>, acTrades, ddType, ddPercent);
    });
